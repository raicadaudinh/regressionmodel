...
---
# Summary
        In this document We'll investigating the relation between fuel consumption(mpg) and tranmission (automatic or manual). The result show that we can conclude between automatic and manual which one is better because weight plays an importance role. The difference of MPG between automatic and manual transmission is not constant and depends on weight.
...
The p value of am is inferior than 0.001 so we reject the hypothesis: coeff=0. Nevertheless, the coefficient of am could be overevaluated. We wonder if adding other variables into our model could improve its power.
